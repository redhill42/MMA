 ...
 * workers along with a number of jobs and a cost matrix which gives the cost
 * of assigning the i'th worker to the j'th job at position (i, j). The goal
 * is to find an assignment of workers to jobs so that no job is assigned more
 * than one worker and so that no worker is assigned to more than one job in
 * such a manner so as to minimize the total cost of completing the jobs.
 * <p>
 *
 ...
 * -1; in no other circumstance will there be unassigned workers. Similarly,
 * an assignment of a cost matrix that has more jobs than workers will necessarily
 * include unassigned jobs; in no other circumstance will there be unassigned
 * jobs. For completeness, an assigment for a square cost matrix will give
...
public class Hungarian {
    ...
    /**
     * Compute aan initial feasible solution by assigning zero labels to the
     ...
     * consists of building a set of matching workers and a set of committed jobs
     ...
     * edges have been exhausted, the labels of committed workers are increated by
     ...
     * decreated by the same amount in order to maintain a feasible labeling).
     * <p>
     *
     ...
     * by maintaining the mimimum slack values among non-committed jobs. When a phase
     * completes, the matching will have increated in size.
     */
    protected void executePhase() {
        while (true) {
            ...
            if (matchWorkerByJob[minSlackJob] == -1) {
                // An augument path has been found.
                ...
                // Update slack values since we increated the size of the commited
                // workers set.
     ...
     * cost matrix is optimal for the original cost matrx.
     */
     ...
     * In addition, update the minimum slacks appropriately.
     */
