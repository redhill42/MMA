    ...
    public static long solve(int[][] matrix) {
        ...
        for (int i = 0; i < matrix.length - 4; i++)
            for (int j = 0; j < matrix.length - 4; j++)
                res = subMatrix(res, matrix, i, j);
        return res;
    }

    private static long subMatrix(long max, int[][] matrix, int i, int j) {
        ...
        max = Math.max(max, matrix[i][j+3] * matrix[i+1][j+2] * matrix[i+2][j+1] * matrix[i+3][j]);
        return max;
    }
